class HistoriaInteractiva {
  constructor() {
    ...
    this.foto = [];
    for (let i = 0; i < this.cantidadTotalPantallas; i++) {
      this.foto[i] = loadImage(`data/imagenes/Imagen${i}.jpg`);
    }
  }

  inicializarRelato() {
    loadStrings("data/texto.txt", (data) => {
      this.relato = data;
    });
  }
  ...
  relatarHistoria(frase, textoX, textoY) {
    ...
    if (this.pantalla === 0) {
      ...
      push();
      textSize(62);
      text(this.relato[frase], textoEnX, textoEnY / 3, 500, 300);
      pop();
    }
    if (this.pantalla > 0) {
      fill(255);
      textSize(14);
      text(this.relato[frase], textoEnX, textoEnY / 3, 500, 600);
    }
  }
